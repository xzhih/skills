...
它不是“看到代码就写文档”的通用触发器。只有当用户明确要求文档驱动、代码/文档同步、架构索引、可追踪文档、全链路文档、UI/UX 操作流程、调用路径图，或同等含义的文档维护工作流时，才应该使用。
...
skill 默认使用用户当前语言。用户明确指定文档语言时，使用指定语言。如果文档服务于多语言团队，agent 在写入持久文件前应先简短复述语言约定。
...
Bootstrap 创建的是一组可以改名、合并、拆分、裁剪的默认文档，而不是强制模板。
...
- 哪些控件或状态进入 loading、disabled、empty、warning、success 或 error
...
Maintenance 应轻量检查这条规则是否仍然存在。如果缺失，并且当前任务包含文档维护，agent 可以补回。如果补回超出当前任务范围，则在最终摘要中说明规则缺失。
