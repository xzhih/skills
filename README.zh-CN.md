...
| `agent-self-driving` | 你显式想让一个长任务持续推进：新项目、新需求、MVP 交付、模型多样化 review/repair、外部 agent 策略或高置信多 agent 收口。 | 以 `dev-flow` 为生命周期主线，调用各 focused owner skill，按需 dispatch debate/review/lanes，保存私有 orchestration 状态，只在真正用户决策或真实 blocker 时暂停。 |
| `agent-requirements-analysis` | 你有一个方向、新项目、新需求、更新或 bug 意图，需要在 Spec 前形成完整需求骨架。 | 按需使用 agent 头脑风暴、同题辩论和讨论治理，先消化 agent 可以回答的开放问题，再返回目标、用户、流程、范围、非目标、约束、风险、confirmed/draft/open 和真正需要用户拍板的问题。 |
| `agent-spec` | 需求已经足够结构化，可以定义实现行为和边界。 | 产出可实现 Spec：行为、范围、非目标、约束、影响面、接口、文档影响、开放问题和 review 状态。 |
| `agent-eval` | Spec 已锁定，需要定义如何证明正确。 | 产出验收检查、自动/人工验证、失败条件、场景、证据要求和风险覆盖。 |
| `agent-plan` | Spec 和 Eval 已锁定，需要可执行任务或 lane 候选。 | 产出任务拆分、依赖、影响面、验证命令、证据要求、风险、停止条件和安全并行 lane 候选。 |
| `agent-debate` | 多个 agent 应该讨论同一份材料和同一个问题：需求、产品摩擦、简单性、必要性、用户流程或方案是否太重。 | 让每个 agent 拿到同一主题和同一材料，综合分歧，把冲突输入后续轮次，只提升有证据支持的结论或开放问题。 |
| `agent-review` | 一个 focused reviewer 或多个 agent 应该审查同一个 artifact：Spec、Eval、plan、design、PR、diff、实现、证据包或最终结果。 | 收集 blocker/major/minor/question，归一化声明，核验证据，并在需要时做 rebuttal 或 recheck。 |
...
| `agent-runtime` | workflow 需要 approved agents、model profiles、capability/runnability checks、OpenCode 或外部 session continuity。 | 应用 runtime gates，并把状态记录在 `docs/dev-flow/capabilities/`。 |
...
Runtime capability/session records 放在 `docs/dev-flow/capabilities/`。
`agent-self-driving` 可以维护 source map、私有 blackboard、raw agent-output
ledger 和 review-repair 连续性。
...
长任务自动化                   -> agent-self-driving
分析需求                       -> agent-requirements-analysis
产出 Spec                      -> agent-spec
产出 Eval                      -> agent-eval
产出 Plan                      -> agent-plan
...
```text
需求分析 -> Spec -> Eval -> Plan -> 执行 -> Review/Test -> Repair/Recheck -> Close
```

| 场景 | 从哪里开始 | 后续怎么走 | 通常跳过或压缩什么 |
| --- | --- | --- | --- |
| 新项目 | 想从方向自动推进到 MVP/完成态时用 `$agent-self-driving`；只想要需求骨架时用 `$agent-requirements-analysis`。 | Orchestration 用 `$dev-flow` 进入 `$agent-requirements-analysis`、`$agent-spec`、`$agent-eval`、`$agent-plan`、review、执行、repair 和收口。需要长期 source-of-truth docs 时，用 `$doc-driven-workflows`。 | 不跳过前期发现，但在风险需要前保持产物轻量。 |
| 新需求 | 想继续推进到实现时用 `$agent-self-driving`；只想做 lifecycle routing 和阶段 checkpoint 时用 `$dev-flow`。 | 用 `$agent-requirements-analysis`、`$agent-spec`、`$agent-eval`、`$agent-plan` 产出文档；用 `$agent-review` 审查；用 `$agent-lanes` 并行执行；文档会漂时用 `$doc-driven-workflows`。 | 需求和 Eval 已清楚时，跳过长讨论。 |
| 修复 | 小而可复现的 bug 可以直接描述。原因、影响面、复现条件或风险不清楚时，用 `$dev-flow`。 | bug 意图不清时用 `$agent-requirements-analysis`，复现/验收证据用 `$agent-eval`，高风险修复计划用 `$agent-plan`，高风险 diff 或最终证据用 `$agent-review`，修复改变文档行为时用 `$doc-driven-workflows`。 | 小修复压缩 Spec/Plan，但保留 Eval/复现和验证。 |
| 更新 | 小更新可以直接描述。行为、架构、范围或风险变化时，用 `$dev-flow`。 | 范围用 `$agent-requirements-analysis`，行为变化用 `$agent-spec`，验收变化用 `$agent-eval`，执行拆分用 `$agent-plan`，变更 artifact 用 `$agent-review`，独立更新面用 `$agent-lanes`。 | 小更新压缩执行流程；行为改变时不要跳过 Eval。 |

速记：

```text
新项目 -> 要自动化用 agent-self-driving；只要骨架用 agent-requirements-analysis
新需求 -> 要交付用 agent-self-driving；只要路由用 dev-flow
...
| 非平凡开发需要需求、Spec、Eval、Plan、执行、review/test、repair 和收口 | `$dev-flow`，它会路由经过 `agent-requirements-analysis`、`agent-spec`、`agent-eval`、`agent-plan` | 没有 Eval gate 就从讨论直接跳到实现 |
| 新项目或新需求需要自动推进到 MVP/完成态或真正暂停点 | `$agent-self-driving` | 可以安全继续时只产出 Requirements/Spec/Eval/Plan 就停下 |
| 方向或想法需要在 Spec 前形成完整需求骨架 | `$agent-requirements-analysis` | 让用户手写第一版完整需求 |
| 需求已经可以变成实现行为 | `$agent-spec` | 把验收测试或任务计划混进 Spec |
| Spec 需要验收检查和证据标准 | `$agent-eval` | 把“完成计划”当成正确性证明 |
| Spec 和 Eval 需要可执行任务或 lane 候选 | `$agent-plan` | 在任务边界和验证方式明确前 dispatch lanes |
...
| 两个或更多明确独立的 work lanes 可以安全并行 | `agent-lanes` | 除非明确需要 Spec/Eval、外部 agents 或 review-repair，否则不要用 `agent-self-driving` |
| 返回的 lane 工作需要证据、范围、冲突和 blocker 分类 | `$agent-lanes`，它会内部使用 `integration-review` | 不检查证据就相信 worker 总结 |
| 显式 Spec/Eval delivery、model-diverse review、外部 agent policy 或反复 review-repair | `$dev-flow` 或 `$agent-self-driving` 做高级控制 | 常规 lane batching 或轻量 goal/path pressure-testing |
...
| 外部 API 文档、SDK 说明、供应商文档、摘录或调研材料需要保留，但还没形成项目判断 | `$discussion-workflows` 的 `inbox/`；形成判断后再进入 `references`、stage artifact 或 `$doc-driven-workflows` | 把 raw material 原样放进长期项目真相 |
...
`agent-review` 和 `integration-review` 的区别：前者让多个 agent 审查同一个
artifact 并产出 artifact-quality verdict；后者审查 returned lane 的范围、diff、
证据、冲突和下一批状态。`integration-review` 发现高风险 artifact 时可以内部调用
`agent-review`，但 lane 状态仍由 `integration-review` 负责。

`agent-self-driving` 是显式入口自动化。它被方向性任务调用时，应该使用
`dev-flow` 和 focused owner skills 一直推进到交付完成、证据阻塞或真正需要用户决策。
它自己的文档应保持私有运行态并链接 owner artifacts，而不是 fork 一套项目真相。
