...
- `agent-improve`：可选的只读代码库或分支审计，用源码证据核实并排序改进项。
...
**`agent-improve`** 是可选的只读顾问：返回排序后的改进项后停止。选中的改进项可从
首个缺失 gate 进入 `dev-flow`；Requirements、Spec、Eval、Plan、实现和审查仍由
现有 owner 负责。
...
| Review + Test | 有已接受 Plan 时先按 Plan 检查实现，再按 Spec 和 Eval 检查，并有可查看的测试或人工验证证据。 |
...
Review 不只是最后一步。`agent-spec` 与 `agent-plan` 在进入 Eval 或执行前必须对整份产物做 `agent-review`。Eval、实现模块、diff 或最终结果在风险需要或需要独立视角时，也用 `agent-review`。由已接受 Plan 产生的实现，在关闭前必须按该 Plan 审查，除非用户明确放弃。
