...
- the user explicitly asks for doc-driven docs, doc sync, architecture indexes, full-chain docs, source-linked docs, operation flows, call paths, code/docs alignment review, or an open-question ledger for doc-driven uncertainty
...
Before writing bootstrap docs, do a small source-backed inventory:
...
Unknowns go into `ledger_path` or a pending section. Do not invent confirmed architecture.

## Maintenance Discovery

Use this bounded discovery order:

1. Inspect the actual change surface: diff, modified files, review target, or user-named files.
...
When code changes would make those docs inaccurate, update the matching docs in the same change.
